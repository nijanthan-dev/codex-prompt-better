...
| Usage/cache | kind, native value/unit, product surface, accounting regime, cache mode/TTL | typed `observations` rows | source-native values only; API and subscription accounting never combine |
| Replay snapshots | plan/budget/policy hashes and bounded host capability states | typed `execution_events` kinds linked to project/session/source | no raw prompt, policy body, host path, or arbitrary payload |
| Attribution/assertions | project state, confidence, validity, evidence, source version | typed `observations` kinds `project_attribution` and `source_assertion` | unknown remains explicit; evidence and provenance are retained |
| Self overhead | `activity_class=governance_overhead` | `observations` with kind `governance_overhead` when an audit revision exists | reported separately; never recursively triggers collection |
