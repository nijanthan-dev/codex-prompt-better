...
func registry() []Definition {
	...
	return []Definition{
		...
		newDefinition("tokens_per_turn", "Tokens per turn", "Show native token workload per completed turn.", "valid total-token deltas / completed turns", "valid total-token deltas", "completed turns", "tokens/turn", PolarityContextual, 1, 0.05, []string{"observations:usage", "turns"}, []string{"incomplete_turns"}, []string{"model", "tokenizer", "workload_class"}, nil, quality, []string{"contextual_not_quality"}, "Must not trigger model or effort downgrade."),
		newDefinition("non_cached_input_per_turn", "Non-cached input per turn", "Measure native non-cached input deltas per completed turn.", "valid non-cached input deltas / completed turns", "valid non-cached input deltas", "completed turns", "tokens/turn", PolarityLowerBetter, 1, 0.05, []string{"observations:usage", "observations:cache", "turns"}, []string{"unknown_cache_accounting"}, []string{"model", "tool_schema", "instruction", "memory", "media"}, []string{"cache_churn", "context_growth"}, quality, []string{"quality_guardrail_required"}, "API and subscription accounting must remain separate."),
		newDefinition("passive_waits_per_100_tool_calls", "Passive waits per 100 tool calls", "Detect avoidable passive polling while preserving required waits.", "classified passive waits / valid logical tool calls * 100", "classified passive waits", "valid logical tool calls", "calls/100_tool_calls", PolarityLowerBetter, 5, 0.10, []string{"tool_calls", "state_epochs", "execution_events:stop"}, []string{"required_external_waits", "user_requested_cadence"}, []string{"external_wait", "approval_state"}, []string{"passive_polling"}, quality, []string{"classification_may_be_unknown"}, "User-requested polling cadence is never waste."),
		...
		newDefinition("validation_presence", "Required validation presence", "Guard that mutations receive structured validation.", "validated mutations / observable mutations", "validated mutations", "observable mutations", "ratio", PolarityGuardrail, 1, 0.01, []string{"state_epochs", "tool_calls", "execution_events:checkpoint"}, nil, []string{"mutation_kind"}, []string{"missing_validation"}, nil, []string{"structured_events_only"}, "Required final/security/release validation is necessary work."),
